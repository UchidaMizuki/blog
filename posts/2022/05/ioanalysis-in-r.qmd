...
ここまでの計算は，[econio](https://github.com/UchidaMizuki/econio)パッケージにまとめられています．また，[econiodatajp](https://github.com/UchidaMizuki/econiodatajp)パッケージを使うと，e-Statから産業連関表をダウンロード・整形する手間なく，そのまま産業連関表を取得できます．以下のように，ここまでの計算と同様の計算を行うことができます（なお，jpioパッケージは現在使用しておらず，econio・econiodatajpに置き換わっています）．
...
特定の最終需要変化に対する経済波及効果は，econioには専用の関数は用意されていないため，上記で紹介したように，レオンチェフ逆行列に最終需要の変化量を掛け合わせることで算出します．
