...
この記事では，jpcityパッケージが内部で使用する市町村合併ネットワークを以下の方針で可視化してみます．

-   1970年4月1日～2024年4月1日にかけての都道府県ごとに市町村合併を可視化
-   市区町村の分離・分割は対象外とする
-   並び替えにおいて時系列は考慮していない（位置を時点に合わせることも可能だが見えづらいため）
...
以下に，市町村合併の可視化結果を示します．データ上の不備等に気づかれた方はコメント等でお知らせいただければ幸いです．
