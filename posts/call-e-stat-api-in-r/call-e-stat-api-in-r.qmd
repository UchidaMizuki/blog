...
ここでは，**年齢階級別の睡眠時間をグラフ化**することを目標として，さきほど取り上げた[睡眠時間に関するデータベース](https://www.e-stat.go.jp/dbview?sid=0003224282)（2015年）からデータを取得してみましょう．まず，必要なパッケージを読み込みます．
...
パイプ演算子`|>` を使うことで，以下のように，`cat01`以外のメタ情報のデータ抽出を続けて行うことができます．
...
データの抽出後に`collect()` を適用することで統計データを取得することができます（また，`collect()`の`n`引数で，取得するデータの列を`"person"`と名付けています）．

ただし，現状では，取得したデータ`data_sleep_2015` に多くの列が存在するため，分析しづらくなっています．**ステップα**で，データ取得とデータ整形を同時に行う方法について説明します．
